...
namespace ShaderUtils
{
    ...
    transposeMatrix(float* matrix)
    {
        if (matrix) {
            ...
            for (int i=0; i<4; i++) {
                for (int j=0; j<4; j++) {
                    temp[i][j] = matrix[4*i+j];
                }
            }
            
            memcpy(matrix, temp, 4 * 4 * sizeof(float));
        }
    }
   ...
   // Apply a rotation
    ...
    multiplyMatrix(float *matrixA, float *matrixB, float *matrixC)
    {
        ...
        memcpy(matrixC, aTmp, 4*4*sizeof(float));
    }
    
    
    // Multiply a vector V with matrix A and write result to vector W
    void multiplyVector(float* vecV, float* matrixA, float* vecW) {
        
        for (int i = 0; i < 4; i++) {
            vecW[i] = 0.0;
            
            for (int j = 0; j < 4; j++) {
                vecW[i] += matrixA[4*i+j] * vecV[j];
            }
        }
    }
